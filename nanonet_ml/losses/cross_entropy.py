...
class CrossEntropyFunction(Function):
    ...
    def _forward(
        ...
        if logits.ndim != 2:
            raise ValueError(
                "CrossEntropyLoss expected 2D logits "
                f"(batch, classes), got shape {logits.shape}."
            )
        ...
        if raw.ndim != 1:
            raise ValueError(
                "CrossEntropyLoss expected integer labels "
                f"with shape ({batch},), received shape {raw.shape}."
            )
        ...
        if (
            ...
            raise ValueError(
                "CrossEntropyLoss targets must contain "
        ...
        if targets.shape != (batch,):
            raise ValueError(
                "CrossEntropyLoss expected integer labels "
                f"with shape ({batch},), received shape {targets.shape}."
            )

        if (
            np.any(targets < 0)
            or np.any(targets >= logits.shape[1])
        ):
            raise ValueError(
                "CrossEntropyLoss labels must be in "
                f"[0, {logits.shape[1] - 1}], "
                f"got min={targets.min()}, max={targets.max()}."
            )

        # log_softmax via log-sum-exp:
