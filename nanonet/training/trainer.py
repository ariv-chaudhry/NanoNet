...
class Trainer:
        ...
        *,
        ...
        for epoch in range(1, epochs + 1):
            ...
            for X_batch, y_batch in loader:
                ...
                if compute_accuracy:
                    total_correct += accuracy(logits, y_batch) * batch_n

            train_loss = total_loss / max(total_examples, 1)
            train_acc = (total_correct / max(total_examples, 1)) if compute_accuracy else None

            val_loss = None
            val_acc = None
            if validation_data is not None:
                X_val, y_val = validation_data
                val_metrics = self._evaluate_raw(X_val, y_val, loss_fn, compute_accuracy)
            ...
            if verbose:
                ...
                if train_acc is not None:
                    msg += f" - accuracy: {train_acc * 100:.2f}%"
                if val_loss is not None:
                    msg += f" - val_loss: {val_loss:.4f}"
                if val_acc is not None:
                    msg += f" - val_accuracy: {val_acc * 100:.2f}%"
                print(msg)
    ...
    def evaluate(
        ...
        *,
        ...
        """
        metrics = self._evaluate_raw(X, y, loss_fn, compute_accuracy, batch_size=batch_size)
    ...
    def _evaluate_raw(
        ...
        loader = DataLoader(TensorDataset(X_arr, y_arr), batch_size=batch_size, shuffle=False)
        ...
        out: dict[str, float] = {}
        if loss_fn is not None:
            out["loss"] = total_loss / max(total_examples, 1)
        if compute_accuracy:
            out["accuracy"] = total_correct / max(total_examples, 1)
        return out
