...
def test_cross_entropy_manual():
    # Single example, two classes:
    # logits [0, 0] -> uniform 0.5 -> -log(0.5)
    logits = Tensor(
        [[0.0, 0.0]],
        requires_grad=True,
    )
...
def test_cross_entropy_rejects_fractional_labels():
    logits = Tensor(
        np.zeros((2, 3)),
        requires_grad=True,
    )
...
def test_cross_entropy_rejects_non_finite_labels():
    logits = Tensor(
        np.zeros((2, 3)),
        requires_grad=True,
    )
